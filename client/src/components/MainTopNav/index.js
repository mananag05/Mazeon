...
const MainTopNav = () => {
    ...
    return (
        ...
        <div className={`ml-auto lg:hidden  `}>
          <div className={` z-10 w-10 h-10 flex mr-5 justify-center flex-col items-center cursor-pointer lg:hidden `} onClick={() => dispatch(toggle())}>
            <div className={`w-6 h-0.5 bg-lighttext rounded-full mt-2 transform transition-all duration-300 `}></div>
            <div className={`w-6 h-0.5 bg-lighttext rounded-full mt-2 transform transition-all duration-300 `}></div>
          </div>
